...
create table if not exists public.members (
  ...
  phone text not null,
  country text not null default 'العراق',
  governorate text,
...
alter table public.members add column if not exists country text not null default 'العراق';
alter table public.members add column if not exists governorate text;
...
update public.members set country = 'العراق' where country is null or btrim(country) = '';

create unique index if not exists members_phone_unique_idx on public.members (phone);
create unique index if not exists members_full_name_unique_idx
on public.members ((lower(btrim(full_name))));
...
drop function if exists public.submit_competition_count(text, text, text, integer);
...
grant execute on function public.submit_competition_count(uuid, text, text, integer)
to anon, authenticated;
...
begin
  ...
  v_next_level := least(coalesce(v_security.violation_level, 0) + 1, 2);
  if v_next_level = 1 then
    v_new_ban_until := null;
  else
...
begin
  ...
  if v_reserved_count >= 5 then
    return query select false, 'الحد الأقصى لحجزك في الختمة الواحدة هو 5 أجزاء فقط.';
    return;
  end if;
